...
<section class="relative hero-clip overflow-hidden" style="min-height:300px;">
    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-20 text-white">
        ...
        <h1 class="text-3xl sm:text-4xl md:text-5xl font-bold mb-4">Partner with NSU Research</h1>
...
<section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
    ...
    {{-- 6-col grid: top 3 each span 2 cols; bottom 2 start at col 2/4 → centered. lg: only so tablet stays 2-col. --}}
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-6 mt-10">
        @foreach($pathways as $p)
        <div class="
            @if($loop->iteration <= 3) lg:col-span-2
            @elseif($loop->iteration === 4) lg:col-start-2 lg:col-span-2
            @else lg:col-start-4 lg:col-span-2
            @endif
            bg-white rounded-2xl border border-gray-100 shadow-sm p-6 card-hover flex flex-col">
            ...
            <a href="{{ route('industry-inquiry') }}" class="btn-outline-teal text-sm text-center">Submit Inquiry</a>
        </div>
        @endforeach
    </div>
</section>
