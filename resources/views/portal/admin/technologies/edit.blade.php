...
@section('content')
<div class="max-w-2xl mx-auto" x-data="{ benefits: {{ json_encode(old('benefits', $technology->benefits ?? [''])) }} }">
    <form method="POST" action="{{ route('portal.admin.technologies.update', $technology) }}" class="space-y-6">
        @csrf
        @method('PATCH')
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="industry_sector" value="{{ old('industry_sector', $technology->industry_sector) }}" required
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500">
                </div>
                <div>
                    ...
                    <select name="development_stage" required class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500">
                        <option value="early_stage" @selected(old('development_stage',$technology->development_stage)==='early_stage')>Early Stage</option>
                        <option value="filed" @selected(old('development_stage',$technology->development_stage)==='filed')>Patent Filed</option>
                        <option value="granted" @selected(old('development_stage',$technology->development_stage)==='granted')>Patent Granted</option>
                        <option value="licensed" @selected(old('development_stage',$technology->development_stage)==='licensed')>Licensed</option>
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Contact Email</label>
                    <input type="email" name="contact_email" value="{{ old('contact_email', $technology->contact_email) }}"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Sort Order</label>
                    <input type="number" name="sort_order" value="{{ old('sort_order', $technology->sort_order) }}" min="0"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="description" rows="4" required
                          class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500">{{ old('description', $technology->description) }}</textarea>
            </div>
            ...
            <div class="flex items-center gap-6">
                <label class="flex items-center gap-2 text-sm text-gray-700">
                    ...
                    <input type="checkbox" name="licensing_available" value="1" @checked(old('licensing_available', $technology->licensing_available))
                           class="rounded border-gray-300 text-teal-600 focus:ring-teal-500">
                    Licensing Available
                </label>
                <label class="flex items-center gap-2 text-sm text-gray-700">
                    <input type="hidden" name="is_published" value="0">
                    <input type="checkbox" name="is_published" value="1" @checked(old('is_published', $technology->is_published))
                           class="rounded border-gray-300 text-teal-600 focus:ring-teal-500">
                    Published
                </label>
            </div>
