...
@section('content')
<div class="max-w-3xl mx-auto" x-data="{ parties: {{ json_encode(old('parties', $agreement->parties ?? [''])) }} }">
    <form method="POST" action="{{ route('portal.agreements.update', $agreement) }}" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PATCH')
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="type" required class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500">
                        ...
                        <option value="{{ $t }}" @selected(old('type',$agreement->type)===$t)>{{ ucwords(str_replace('_',' ',$t)) }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="status" required class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500">
                        ...
                        <option value="{{ $s }}" @selected(old('status',$agreement->status)===$s)>{{ ucwords(str_replace('_',' ',$s)) }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="date" name="expiry_date" value="{{ old('expiry_date', $agreement->expiry_date?->format('Y-m-d')) }}"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500">
                </div>
                <div>
                    ...
                    <select name="disclosure_id" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500">
                        ...
                        <option value="{{ $d->id }}" @selected(old('disclosure_id',$agreement->disclosure_id)==$d->id)>{{ $d->disclosure_id }} — {{ Str::limit($d->title, 50) }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="patent_id" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500">
                        ...
                        <option value="{{ $p->id }}" @selected(old('patent_id',$agreement->patent_id)==$p->id)>{{ $p->patent_number ?? 'Draft' }} — {{ Str::limit($p->title, 50) }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
                ...
                <div class="space-y-2">
                    <template x-for="(party, i) in parties" :key="i">
                        <div class="flex gap-2">
                            ...
                            <button type="button" @click="if(parties.length > 1) parties.splice(i, 1)" class="px-2 text-gray-400 hover:text-red-500">×</button>
                        </div>
                    </template>
                </div>
            </div>
            ...
            @endif
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Replace Document (optional)</label>
                <input type="file" name="document" accept=".pdf,.doc,.docx" class="block text-sm text-gray-600">
            </div>
        </div>
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('portal.agreements.show', $agreement) }}" class="text-sm text-gray-500 hover:underline">Cancel</a>
        </div>
    </form>
</div>
@endsection
